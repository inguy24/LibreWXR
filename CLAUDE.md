...
- **BBOX (`LIBREWXR_BBOX`):** Geographic bounding box as `south,west,north,east` (decimal degrees). Controls radar region cropping, satellite source selection, alert filtering, and tile rendering extent. The value means exactly what it says — no expansion is applied. `get_bbox()` in `config.py` parses the raw value. When absent, LibreWXR renders the full region (e.g., full CONUS). Current deploy: `LIBREWXR_BBOX=26.75,-129.5,40.75,-105.5` (SoCal + surrounding states). The dashboard uses these bounds for map zoom extents via the capability response.
...
- **Multi-satellite compositing (ADR-079):** When `LIBREWXR_MULTI_SATELLITE=true` (default), each satellite family is enabled if its geostationary disk overlaps the BBOX (checked via `bbox_overlaps_disk()` in `_geo_base.py` using the forward projection — not longitude thresholds). This correctly handles: GOES-18/19 CONUS sector scan limits (curved boundary in lat/lon space, e.g. GOES-18's eastern edge is ~-110°W at 27°N, ~-105°W at 37°N), Pacific BBOXes with negative longitudes (Hawaii at -160°W → Himawari enabled), and any BBOX that straddles coverage boundaries. `_find_all_satellite_families()` in `api/routes.py` returns all loaded families (not just first). `render_multi_satellite_tile()` in `tiles/satellite_renderer.py` composites per-pixel; in overlap zones, prefers the source whose sub-satellite longitude is closest to the pixel's longitude. Cache keys include sorted family names. Tile warmer iterates all families. Memory: +56 MB per additional family. `LIBREWXR_MULTI_SATELLITE=false` preserves center-only behavior
- **Memory management:** Radar frames, ECMWF grids, and nowcast data use numpy memmap (temp files); radar fetcher skips timestamps already in store. Geostationary satellite ingest dedupes its S3 listing to latest-key-per-timestamp (NOAA zero-pads the `_c` creation token, so the lexicographically largest key is the most recently created — a republished/reprocessed scan wins) and trims to the newest `max_frames` distinct timestamps BEFORE downloading — keys that cannot survive retention are never fetched (the listing window stays generous for restart/gap refill). A resident timestamp whose listed key is newer than the recorded one is re-downloaded and replaced; the fetcher then invalidates that timestamp's satellite tile-cache entries (both key shapes) before the warm fires, so corrected tiles re-render. Frames restored from the disk cache have no recorded key and adopt the listed key without re-downloading. Docker image uses jemalloc (`LD_PRELOAD`) instead of glibc malloc — jemalloc's per-size-class arenas handle fragmentation from numpy array churn dramatically better, reducing steady-state RSS by 30-50%. `release_memory()` in `memory.py` calls `gc.collect()` after heavy operations.
