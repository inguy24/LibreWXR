...
- **Memory management:** Radar frames, ECMWF grids, and nowcast data use numpy memmap (temp files); radar fetcher skips timestamps already in store. Geostationary satellite ingest dedupes its S3 listing to first-key-per-timestamp and trims to the newest `max_frames` distinct timestamps BEFORE downloading — keys that cannot survive retention are never fetched (the listing window stays generous for restart/gap refill). Docker image uses jemalloc (`LD_PRELOAD`) instead of glibc malloc — jemalloc's per-size-class arenas handle fragmentation from numpy array churn dramatically better, reducing steady-state RSS by 30-50%. `release_memory()` in `memory.py` calls `gc.collect()` after heavy operations.
