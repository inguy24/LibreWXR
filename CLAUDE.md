...
- **Multi-satellite compositing (ADR-079):** When `LIBREWXR_MULTI_SATELLITE=true` (default), auto-selection is BBOX-edge-aware — checking west/east edges, not just center longitude. A BBOX spanning the -100° GOES-18/19 split enables both families. `_find_all_satellite_families()` in `api/routes.py` returns all loaded families (not just first). `render_multi_satellite_tile()` in `tiles/satellite_renderer.py` composites per-pixel with first-valid-pixel selection; in overlap zones, prefers the source whose sub-satellite longitude is closest to the pixel's longitude. Cache keys include sorted family names. Tile warmer iterates all families. Memory: +56 MB per additional family. `LIBREWXR_MULTI_SATELLITE=false` preserves center-only behavior
