...
src/librewxr/
  ...
  tiles/
    ...
    warmer.py        # Background tile pre-rendering (radar + satellite). warm_satellite() pre-renders satellite tiles after each ingest cycle. warm_satellite_demand() provides demand-driven warming — on satellite tile cache miss, pre-renders all other timestamps at the same (z,x,y). Deploy: VIS disabled (`GOES_VIS_ENABLED=false`), IR-only
  colors/
