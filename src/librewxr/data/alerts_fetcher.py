...
async def _resolve_zone_geometries(
        ...
        async with sem:
            try:
                ...
                if resp.status_code != 200:
                    logger.debug("NWS zone %s returned %d", url, resp.status_code)
    ...
    if to_fetch:
        logger.debug(
            ...
            len(to_fetch), len(zone_urls) - len(to_fetch),
        )
        await asyncio.gather(*(fetch_zone(url) for url in to_fetch))

    return {url: _zone_geometry_cache.get(url) for url in zone_urls}
...
class WMOAlertsFetcher:
    ...
    async def _fetch_nws_alerts(self) -> list[AlertEntry]:
        ...
        # Track null-geometry alerts for zone resolution
        ...
        for feature in data.get("features", []):
            ...
            # Zone-based alerts (geometry null) carry affectedZones URLs
            # whose GeoJSON we can resolve to build a polygon.
            if polygon is None:
        ...
        # Batch-resolve zone geometries for null-geometry alerts
        if all_zone_urls:
            ...
            logger.info(
                ...
                resolved_count, len(all_zone_urls),
                enriched, len(alerts_needing_zones),
            )
